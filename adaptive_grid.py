...
# Обёртка над reconstruct для тензоров
def postprocess(tensor: Tensor) -> Tensor:
    img = (tensor.squeeze().numpy() * 255).astype(np.uint8)
    out = reconstruct(img)
    return torch.from_numpy(out.astype(np.float32) / 255.0).reshape(tensor.shape)
